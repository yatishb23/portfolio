  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const totalSubmissions = filtered.reduce(
          (sum, item) => sum + item.count,
          0,
        );
        const totalActiveDays = filtered.filter(
          (item) => item.count > 0,
        ).length;
            ...
            <div key={i} className="flex flex-col gap-2">
              <div className="grid grid-rows-7 gap-1.5">
                {Array.from({ length: 7 }).map((_, j) => (
                  <Skeleton
                    key={j}
                    className="h-3.5 w-3.5 rounded-sm bg-neutral-200 dark:bg-neutral-800"
                  />
                ))}
              </div>
      ...
      <div className="relative">
          ...
          <div className="flex gap-4 min-w-max">
            {months.map((month, idx) => (
              ...
              >
                          ...
                          !d.date ? (
                            ...
                            <div
                              ...
                              className={`w-3.5 h-3.5 rounded-[3px] ${getColorClass(
                                heatmapData.find((item) => item.date === d.date)
                                  ?.count || 0,
                ...
                <div className="text-[10px] font-normal uppercase tracking-widest text-neutral-400 dark:text-neutral-600 mt-3 text-center">
                  {month.name}
                </div>
              </div>
            ))}
        ...
        <div className="mt-6 pt-6 border-t border-neutral-100 dark:border-neutral-900 flex items-center justify-between gap-3">
          <div className="h-6 flex items-center">
            {hoveredData ? (
              <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2 duration-300">
                <div
                  className={`w-2 h-2 rounded-full ${getColorClass(hoveredData.count)}`}
                />
                <span className="text-[10px] font-normal text-neutral-950 dark:text-neutral-50 uppercase tracking-widest">
                  {hoveredData.count} Submissions on{" "}
              ...
              <span className="text-[10px] font-normal uppercase tracking-[0.2em] text-neutral-300 dark:text-neutral-700">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-normal uppercase tracking-widest text-neutral-400">
              Less
            </span>
